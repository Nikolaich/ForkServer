...
func handler(w http.ResponseWriter, r *http.Request) {
	defer func() {
		if e := recover(); e != nil {
			Err.Println(e, "ON REQUEST:", r.RequestURI)
			switch v := e.(type) {
			case int:
	...
	Inf.Println("REQUEST:", r.RequestURI)
...
func handleSvc(w http.ResponseWriter, r *http.Request, n string) {
	...
	case "test":
		...
		w.Write([]byte("<html><h1>ForkPlayer DLNA Work!</h1><b>" + Name + " v. " + Vers + " with plugins v. " + sets.PlugsTag + "</b><p>Up: " + ((time.Since(stime) / time.Second) * time.Second).String() + "</p></html>"))
	case "test.json":
		sets.stata(w)
	case "set":
		sets.set(w, r)
	...
	case "gc":
		...
		runtime.GC()
		mutex.Unlock()
		w.Write([]byte("Garbage collection completed!"))
	case "restart":
		go Restart()
		w.Write([]byte("Restarted within a seconds!"))
	default:
