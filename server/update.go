...
func updateFS(justCheck bool) (nv bool) {
	...
	if i, e = gitInfo(gitFS); e != nil {
		...
		Wrn.Println(t, "there is no update.")
	} else if i.U = i.getAss(); i.U == "" {
		Wrn.Println(t, "there is no distrib for", runtime.GOOS, "/", runtime.GOARCH, "!")
		nv = false
	} else if justCheck {
		return
...
func updatePS(justCheck bool) (nv bool) {
	...
	if i, e := gitInfo(gitPlugs); e != nil {
		...
		Wrn.Println("Plugins has been updated to v.", sets.PlugsTag)
	}
	if rm {
